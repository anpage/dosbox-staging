...
static inline Bit32u swapByte32(Bit32u x)
{
	return (((x >> 24) & 0x000000ff) | ((x >> 8) & 0x0000ff00) |
	        ((x << 8) & 0x00ff0000) | ((x << 24) & 0xff000000));
}

static inline Bit16u swapByte16(Bit16u x)
{
	return (x >> 8) | (x << 8);
}
...
static void gdbSetEip(Bit32u gdb_eip)
{
	...
	switch (gdb_eip_mode) {
	case GdbFlatEip: reg_eip = gdb_eip - SegPhys(cs); break;
	case GdbRealEip: reg_eip = gdb_eip; break;
	}
}
...
{
	switch (gdb_eip_mode) {
	case GdbFlatEip: return reg_eip + SegPhys(cs);
	case GdbRealEip: return reg_eip;
	}
}
...
{
	switch (reg) {
	case 0: return swapByte32(reg_eax);
	case 1: return swapByte32(reg_ecx);
	case 2: return swapByte32(reg_edx);
	case 3: return swapByte32(reg_ebx);
	case 4: return swapByte32(reg_esp);
	case 5: return swapByte32(reg_ebp);
	case 6: return swapByte32(reg_esi);
	case 7: return swapByte32(reg_edi);
	case 8: return swapByte32(gdbGetEip());
	case 9: return swapByte32(reg_flags);
	case 10: return swapByte32(SegValue(cs));
	case 11: return swapByte32(SegValue(ss));
	case 12: return swapByte32(SegValue(ds));
	case 13: return swapByte32(SegValue(es));
	case 14: return swapByte32(SegValue(fs));
	case 15: return swapByte32(SegValue(gs));
	default: return 0;
	}
}
...
{
	switch (reg) {
	case 0: reg_eax = swapByte32(value); break;
	case 1: reg_ecx = swapByte32(value); break;
	case 2: reg_edx = swapByte32(value); break;
	case 3: reg_ebx = swapByte32(value); break;
	case 4: reg_esp = swapByte32(value); break;
	case 5: reg_ebp = swapByte32(value); break;
	case 6: reg_esi = swapByte32(value); break;
	case 7: reg_edi = swapByte32(value); break;
	case 8: gdbSetEip(swapByte32(value)); break;
	case 9: reg_flags = swapByte32(value); break;
	case 10: SegSet16(cs, swapByte32(value)); break;
	case 11: SegSet16(ss, swapByte32(value)); break;
	case 12: SegSet16(ds, swapByte32(value)); break;
	case 13: SegSet16(es, swapByte32(value)); break;
	case 14: SegSet16(fs, swapByte32(value)); break;
	case 15: SegSet16(gs, swapByte32(value)); break;
	}
}
...
{
	if (set_) {
		pair<bp_map_t::iterator, bool> ret = b.insert(
		        bp_map_t::value_type(addr, len));
...
void DEBUG_GdbProcessMonitorPacket(char *data)
{
	...
	for (char *t = strtok_r(data, delim, &save); t != NULL;
	...
	DEBUG_ShowMsg("  monitor cycle_bp [value]           - set relative "
	              "cycle breakpoint.");
	DEBUG_ShowMsg("  monitor cycle_abs_bp [value]       - set absolute "
	              "cycle breakpoint.");
	DEBUG_ShowMsg("  monitor flat_eip [ 0 | 1 ]         - enable/disable "
	              "use of flat eip register value.");
	DEBUG_ShowMsg("  monitor int_bp [int_num] [ 0 | 1 ] - set breakpoint "
	              "on cpu interrupt.");
	DEBUG_ShowMsg("  monitor log_gdt                    - Lists "
	              "descriptors of the GDT.");
	DEBUG_ShowMsg("  monitor log_ldt                    - Lists "
	              "descriptors of the LDT.");
	DEBUG_ShowMsg("  monitor log_idt                    - Lists "
	              "descriptors of the IDT.");
	DEBUG_ShowMsg("  monitor log_cpuinfo                - Display CPU "
	              "status information.");
	DEBUG_ShowMsg("  monitor log_pages [page]           - Display content "
	              "of page table.");
	DEBUG_ShowMsg("  monitor remote_debug [ 0 | 1 ]     - enable/disable "
	              "gdb remote protocol debug.");
	DEBUG_ShowMsg("  monitor set_log_heavy [ 0 | 1 ]    - enable/disable "
	              "heavy CPU logging.");
	DEBUG_ShowMsg("  monitor write_log_instruction      - write "
	              "instructions log to disk.");
...
void DEBUG_GdbProcessPackets()
{
	...
	case 'M': { // write memory
		...
		if (len % 4 == 0 && addr % 4 == 0) { // use dword access
			for (unsigned int i = 0; i < len / 4; i++) {
				...
				err |= mem_writed_checked(
				        addr + i, swapByte32(strtoul(hex, 0, 16)));
			}

		} else if (len % 2 == 0 && addr % 2 == 0) { // use word access
			for (unsigned int i = 0; i < len / 2; i++) {
				memcpy(hex, end + 1 + i * 4, 4);
				err |= mem_writew_checked(
				        addr + i, swapByte16(strtoul(hex, 0, 16)));
			}

		} else { // use byte access
			for (unsigned int i = 0; i < len; i++) {
				memcpy(hex, end + 1 + i * 2, 2);
				err |= mem_writeb_checked(addr + i,
				                          strtoul(hex, 0, 16));
			}
		}
	...
	case 'c': { // continue [optional resume addr in hex]
		if (data[1]) {
			...
			// go through single step to avoid break-point on resume
			// address
			gdb_state = GdbStep;
			step_next_state = GdbRunning;
			step_eip = gdbGetEip();
	...
	case 'Z': {
		...
		case '2': // write watch point
			DEBUG_GdbPointSet(write_watch_points, addr, len,
			                  data[0] == 'Z');
			break;
		...
		case '3': // read watch point
			DEBUG_GdbPointSet(read_watch_points, addr, len,
			                  data[0] == 'Z');
			break;

		default: DEBUG_GdbWritePacket(""); return;
		}
...
static bool DEBUG_GdbCheckEvent(void)
{
	...
	case GdbNotConnected: // check for incoming connection
		...
		if (poll(&pf, 1, 0) > 0) {
			...
			gdb_asocket = accept(gdb_socket, (struct sockaddr *)&addr,
			                     &addr_size);
...
void DEBUG_ShowMsg(char const *format, ...)
{
	...
	if (gdb_state == GdbRunning || gdb_state == GdbStep ||
	    gdb_state == GdbMonitor) {
		if (buf[0]) {
...
bool DEBUG_HeavyIsBreakpoint(void)
{
	...
	if (gdb_state == GdbStep &&
	    (step_eip != gdbGetEip() || step_cs != SegValue(cs))) {
	...
	/* sometimes check for incoming gdb client connections or CTRL-C from
	 * client */
...
{
	if ((gdb_state == GdbRunning || gdb_state == GdbStep) &&
	    DEBUG_GdbPointCheck(read_watch_points, address)) {
		DEBUG_ShowMsg("GDB: %u hit a memory read access watchpoint: "
		              "address=0x%08x.\n",
...
{
	if ((gdb_state == GdbRunning || gdb_state == GdbStep) &&
	    DEBUG_GdbPointCheck(write_watch_points, address)) {
		DEBUG_ShowMsg("GDB: %u: hit a memory write access watchpoint: "
		              "address=0x%08x, new_value=0x%x.\n",
...
void DEBUG_Init(Section *sec)
{
	...
	int tmp = 1;
#ifdef WIN32
	setsockopt(gdb_socket, SOL_SOCKET, SO_REUSEADDR, (const char *)tmp,
	           sizeof(tmp));
#else
	...
	for (int i = 0;; i++) {
		...
		if (bind(gdb_socket, (struct sockaddr *)&addr,
