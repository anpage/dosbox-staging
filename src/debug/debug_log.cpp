...
static void DEBUG_LogMCBChain(Bit16u mcb_segment) {
	DOS_MCB mcb(mcb_segment);
	char filename[9]; // 8 characters plus a terminating NUL
	const char *psp_seg_note;
	Bit16u DOS_dataOfs = static_cast<Bit16u>(DEBUG_dataOfs); //Realmode addressing only
	PhysPt dataAddr = PhysMake(DEBUG_dataSeg,DOS_dataOfs);// location being viewed in the "Data Overview"
	...
	while (true) {
		...
		if (dataAddr >= mcbStartAddr && dataAddr < mcbEndAddr) {
			LOG(LOG_MISC,LOG_ERROR)("   (data addr %04hX:%04X is %u bytes past this MCB)",DEBUG_dataSeg,DOS_dataOfs,dataAddr - mcbStartAddr);
		}
...
static void DEBUG_LogMCBS(void)
{
...
void LOG::operator() (char const* format, ...){
	...
	DEBUG_ShowMsg("%10u: %s:%s\n",static_cast<Bit32u>(DEBUG_cycle_count),loggrp[d_type].front,buf);
}
...
#if C_HEAVY_DEBUG || C_GDBSERVER
static void DEBUG_LogInstruction(Bit16u segValue, Bit32u eipValue,  ofstream& out) {
	static char empty[23] = { 32,32,32,32,32,32,32,32,32,32,32,32,32,32,32,32,32,32,32,32,32,32,0 };

	if (DEBUG_cpuLogType == 3) { //Log only cs:ip.
